...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--train_data', type=str, default='abu-beach-1', help='Name of train set.')
    parser.add_argument('--test_data', type=str, default='abu-beach-1', help='Name of test set.')
...
def make_rgb_image(data):
    """Convert HSI data to an RGB image for visualization."""
    if data.ndim == 2:
        data = np.expand_dims(data, axis=-1)

    channels = data.shape[-1]
    if channels >= 3:
        band_ids = np.linspace(0, channels - 1, 3).astype(int)
        rgb = data[:, :, band_ids]
    else:
...
def save_figure(original_data, gt_map, anomaly_map, auc_value, save_path):
    """Save a 1x3 figure for original image, ground truth, and anomaly map."""
    original_img = make_rgb_image(original_data)
    gt_img = np.squeeze(gt_map)
    anomaly_img = np.squeeze(anomaly_map)

    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    fig.suptitle('AUC = {:.6f}'.format(float(auc_value)), fontsize=14)
    ...
    anomaly_plot = axes[2].imshow(anomaly_img, cmap='jet')
    axes[2].set_title('HSI_anomaly')
    axes[2].axis('off')
    fig.colorbar(anomaly_plot, ax=axes[2], fraction=0.046, pad=0.04)
...
if __name__ == '__main__':
    ...
    if args.RX:
        ...
        dataset_name = args.test_data if args.test_data != 'None' else file_name
        rx_result = run_rx(x)
        ...
        figure_save_path = os.path.join(
            ...
            'figure-{}-t{}-K{}_AUC_{:.6f}.png'.format(dataset_name, args.t, args.K, auc)
        )
        save_figure(test_data, test_map, rx_result_bs, auc, figure_save_path)
        print('Saved figure to {}'.format(figure_save_path))
